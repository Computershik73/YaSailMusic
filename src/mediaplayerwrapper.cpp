...
QString MediaPlayerWrapper::currentIndexSt() const {
    if (_player->playlist() == NULL) return "-1";
    return QString::number(_player->playlist()->currentIndex());
}
...
int MediaPlayerWrapper::queue() const
{
    return _queue;
}

void MediaPlayerWrapper::setQueue(int queue)
{
    _queue = queue;
}
